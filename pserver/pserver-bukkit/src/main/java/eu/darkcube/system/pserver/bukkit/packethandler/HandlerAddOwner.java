...
public class HandlerAddOwner implements PacketHandler<PacketAddOwner> {
    @Override
    public Packet handle(PacketAddOwner packet) throws Throwable {
        InjectionLayer.boot().instance(EventManager.class).callEvent(new PServerAddOwnerEvent(PServerProvider.instance().pserver(packet.id()).get(), packet.owner()));
        return null;
    }
}
