...
public class PacketManager {
	...
	public ITask<Packet> sendPacketQueryAsync(Packet packet, ServiceInfoSnapshot snapshot) {
		return preparePacket(packet, snapshot).sendSingleQueryAsync()
				.map(m -> PacketSerializer.getPacket(m.getJson(), PacketSerializer.getClass(m.getJson())));
	}
	...
	public <T extends Packet> ITask<T> sendPacketQueryAsync(Packet packet, Class<T> responsePacketType) {
		return preparePacket(packet).sendSingleQueryAsync()
	...
	private ChannelMessage preparePacket(Packet packet, ServiceInfoSnapshot snapshot) {
		return ChannelMessage.builder()
				.targetService(snapshot.getName())
				.channel(PServer.CHANNEL)
				.json(prepareDocument(packet))
				.build();
	}

	private ChannelMessage preparePacket(Packet packet) {
		return ChannelMessage.builder().targetAll().channel(PServer.CHANNEL).json(prepareDocument(packet)).build();
	}
	...
	public class Listener {
		...
		public void handle(ChannelMessageReceiveEvent e) {
			if (e.getChannel() == null || !e.getChannel().equals(PServer.CHANNEL)) {
				return;
			}
			try {
