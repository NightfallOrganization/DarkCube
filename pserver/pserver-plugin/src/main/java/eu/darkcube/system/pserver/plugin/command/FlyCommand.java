    ...
    public FlyCommand() {
        super("fly", new String[0], b -> b.executes(context -> {
            return fly(context, Collections.singleton(context.getSource().asPlayer()), null);
                ...
                .then(Commands
                        .argument("state", EnumArgument.enumArgument(State.values(), s -> new String[]{s.name().toLowerCase()}))
                        .executes(context -> {
    ...
    private static int fly(CommandContext<CommandSource> context, Collection<Player> targets, Boolean state) {
        targets.forEach(p -> {
            ...
            if (p.getAllowFlight() != newState) {
                ...
                context
                        .getSource()
                        .sendMessage(Message.FLIGHT_CHANGED_SINGLE, p.getName(), p.getAllowFlight() ? Message.ON.getMessageString(context
                                .getSource()
                                .getSource()) : Message.OFF.getMessageString(context.getSource().getSource()));
            }
        });
        return 0;
    }
