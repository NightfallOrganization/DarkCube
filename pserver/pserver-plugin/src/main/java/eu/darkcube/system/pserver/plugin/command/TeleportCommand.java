...
public class TeleportCommand extends PServerExecutor {
    ...
    public TeleportCommand() {
        super("teleport", new String[]{"tp"}, b -> b.then(Commands.argument("targets", EntityArgument.entities()).executes(source -> {
            ...
            return TeleportCommand.teleportToEntity(source.getSource(), EntityArgument.getEntities(source, "targets"), EntityArgument.getEntity(source, "destination"));
                ...
                .executes(source -> {
                    return TeleportCommand.teleportToPos(source.getSource(), EntityArgument.getEntities(source, "targets"), source
                            .getSource()
                            .getWorld(), Vec3Argument.getLocation(source, "location"), null, null);
                })
                .then(Commands.argument("rotation", RotationArgument.rotation()).executes(source -> {
                    return TeleportCommand.teleportToPos(source.getSource(), EntityArgument.getEntities(source, "targets"), source
                            .getSource()
                            .getWorld(), Vec3Argument.getLocation(source, "location"), RotationArgument.getRotation(source, "rotation"), null);
                }))
                .then(Commands
                        .literal("facing")
                        .then(Commands
                                .literal("entity")
                                .then(Commands.argument("facingEntity", EntityArgument.entity()).executes(source -> {
                                    return TeleportCommand.teleportToPos(source.getSource(), EntityArgument.getEntities(source, "targets"), source
                                            .getSource()
                                            .getWorld(), Vec3Argument.getLocation(source, "location"), null, new Facing(EntityArgument.getEntity(source, "facingEntity"), EntityAnchorArgument.Type.FEET));
                                }).then(Commands.argument("facingAnchor", EntityAnchorArgument.entityAnchor()).executes(source -> {
                                    return TeleportCommand.teleportToPos(source.getSource(), EntityArgument.getEntities(source, "targets"), source
                                            .getSource()
                                            .getWorld(), Vec3Argument.getLocation(source, "location"), null, new Facing(EntityArgument.getEntity(source, "facingEntity"), EntityAnchorArgument.getEntityAnchor(source, "facingAnchor")));
                                }))))
                        .then(Commands.argument("facingLocation", Vec3Argument.vec3()).executes(source -> {
                            return TeleportCommand.teleportToPos(source.getSource(), EntityArgument.getEntities(source, "targets"), source
                                    .getSource()
                                    .getWorld(), Vec3Argument.getLocation(source, "location"), null, new Facing(Vec3Argument.getVec3(source, "facingLocation")));
                        }))))).then(Commands.argument("location", Vec3Argument.vec3()).executes(source -> {
            return TeleportCommand.teleportToPos(source.getSource(), Collections.singleton(source.getSource().assertIsEntity()), source
                    .getSource()
                    .getWorld(), Vec3Argument.getLocation(source, "location"), null, null);
        }).then(Commands.argument("rotation", RotationArgument.rotation()).executes(source -> {
            return TeleportCommand.teleportToPos(source.getSource(), Collections.singleton(source.getSource().assertIsEntity()), source
    ...
    public static int teleportToPos(CommandSource source, Collection<? extends Entity> targets, World world, ILocationArgument position, final ILocationArgument rotation, Facing facing) {
        ...
        for (Entity entity : targets) {
            if (vector2f == null) {
                TeleportCommand.teleport(source, entity, world, vector3d.x, vector3d.y, vector3d.z, entity.getLocation().getYaw(), entity
                        .getLocation()
                        .getPitch(), facing);
            } else {
        ...
        if (targets.size() == 1) {
            source.sendMessage(Message.TELEPORTED_TO_LOCATION_SINGLE, targets
                    .stream()
                    .findAny()
                    .get()
                    .getName(), vector3d.x, vector3d.y, vector3d.z);
        } else {
    ...
    private static void lookAt(Entity entity, EntityAnchorArgument.Type anchor, Vector3d target) {
        ...
        float rotationPitch = MathHelper.wrapDegrees((float) (-(MathHelper.atan2(d1, d3) * (180F / (float) Math.PI))));
        float rotationYaw = MathHelper.wrapDegrees((float) (MathHelper.atan2(d2, d0) * (180F / (float) Math.PI)) - 90.0F);
