...
public class TeleportCommand extends PServerExecutor {
	...
	public TeleportCommand() {
		super("teleport", new String[] {"tp"}, b -> b.then(
						Commands.argument("targets", EntityArgument.entities()).executes(source -> {
							// When tp @e[limit=1] for example
							return TeleportCommand.teleportToSingleEntity(source.getSource(),
									EntityArgument.getEntities(source, "targets"));
						}).then(Commands.argument("destination", EntityArgument.entity())
								.executes(source -> {
									return TeleportCommand.teleportToEntity(source.getSource(),
											EntityArgument.getEntities(source, "targets"),
											EntityArgument.getEntity(source, "destination"));
								})).then(Commands.argument("location", Vec3Argument.vec3())
								.executes(source -> {
									return TeleportCommand.teleportToPos(source.getSource(),
											EntityArgument.getEntities(source, "targets"),
											source.getSource().getWorld(),
											Vec3Argument.getLocation(source, "location"), null, null);
								}).then(Commands.argument("rotation", RotationArgument.rotation())
										.executes(source -> {
											return TeleportCommand.teleportToPos(source.getSource(),
													EntityArgument.getEntities(source, "targets"),
													source.getSource().getWorld(),
													Vec3Argument.getLocation(source, "location"),
													RotationArgument.getRotation(source, "rotation"), null);
												...
												.then(Commands.argument("facingEntity", EntityArgument.entity())
														.executes(source -> {
															return TeleportCommand.teleportToPos(source.getSource(),
																	...
																	Vec3Argument.getLocation(source, "location"),
																	null, new Facing(
																			EntityArgument.getEntity(source,
																					"facingEntity"),
																			EntityAnchorArgument.Type.FEET));
														}).then(Commands.argument("facingAnchor",
																		EntityAnchorArgument.entityAnchor())
																.executes(source -> {
																	return TeleportCommand.teleportToPos(
																			source.getSource(),
																			EntityArgument.getEntities(source,
																					"targets"),
																			source.getSource().getWorld(),
																			Vec3Argument.getLocation(source,
																					"location"), null, new Facing(
																					EntityArgument.getEntity(source,
																							"facingEntity"),
																					EntityAnchorArgument.getEntityAnchor(
																							source,
																							"facingAnchor")));
																}))))
										.then(Commands.argument("facingLocation", Vec3Argument.vec3())
												.executes(source -> {
													return TeleportCommand.teleportToPos(source.getSource(),
															...
															source.getSource().getWorld(),
															Vec3Argument.getLocation(source, "location"),
															null, new Facing(Vec3Argument.getVec3(source,
																	"facingLocation")));
												})))))
				...
				.then(Commands.argument("location", Vec3Argument.vec3()).executes(source -> {
					return TeleportCommand.teleportToPos(source.getSource(),
							Collections.singleton(source.getSource().assertIsEntity()),
							source.getSource().getWorld(),
							Vec3Argument.getLocation(source, "location"), null, null);
				}).then(Commands.argument("rotation", RotationArgument.rotation())
						.executes(source -> {
	...
	private static int teleportToSingleEntity(CommandSource source,
			Collection<? extends Entity> destinations) throws CommandSyntaxException {
		...
		source.sendMessage(Message.TELEPORTED_TO_ENTITY_SINGLE, source.assertIsEntity().getName(),
				destination.getName());
		return 0;
	}
	...
	private static int teleportToEntity(CommandSource source, Collection<? extends Entity> targets,
		...
		for (Entity entity : targets) {
			TeleportCommand.teleport(source, entity, destination.getWorld(), loc.getX(), loc.getY(),
					loc.getZ(), loc.getYaw(), loc.getPitch(), null);
		}
		if (targets.size() == 1) {
			source.sendMessage(Message.TELEPORTED_TO_ENTITY_SINGLE,
					targets.stream().findAny().get().getName(), destination.getName());
		} else {
			source.sendMessage(Message.TELEPORTED_TO_ENTITY_MULTIPLE, targets.size(),
					destination.getName());
		}
		return targets.size();
	}

	public static int teleportToPos(CommandSource source, Collection<? extends Entity> targets,
			World world, ILocationArgument position, final ILocationArgument rotation,
			Facing facing) {
		Vector3d vector3d = position.getPosition(source);
		final Vector2f vector2f = rotation == null ? null : rotation.getRotation(source);

		for (Entity entity : targets) {
			if (vector2f == null) {
				TeleportCommand.teleport(source, entity, world, vector3d.x, vector3d.y, vector3d.z,
						entity.getLocation().getYaw(), entity.getLocation().getPitch(), facing);
			} else {
				TeleportCommand.teleport(source, entity, world, vector3d.x, vector3d.y, vector3d.z,
						vector2f.x, vector2f.y, facing);
			}
		}

		if (targets.size() == 1) {
			source.sendMessage(Message.TELEPORTED_TO_LOCATION_SINGLE,
					targets.stream().findAny().get().getName(), vector3d.x, vector3d.y, vector3d.z);
		} else {
			source.sendMessage(Message.TELEPORTED_TO_LOCATION_MULTIPLE, targets.size(), vector3d.x,
	...
	private static void teleport(CommandSource source, Entity entity, World world, double x,
			double y, double z, float yaw, float pitch, Facing facing) {
		entity.teleport(new Location(world, x, y, z, yaw, pitch));
		if (facing != null)
			facing.updateLook(source, entity);
	}

	private static void lookAt(Entity entity, EntityAnchorArgument.Type anchor, Entity target,
			EntityAnchorArgument.Type targetAnchor) {
	...
	private static void lookAt(Entity entity, EntityAnchorArgument.Type anchor, Vector3d target) {
		...
		float rotationPitch = MathHelper.wrapDegrees(
				(float) (-(MathHelper.atan2(d1, d3) * (180F / (float) Math.PI))));
		float rotationYaw = MathHelper.wrapDegrees(
		...
		public void updateLook(CommandSource source, Entity entityIn) {
			if (this.entity != null) {
				if (entityIn instanceof Player) {
					TeleportCommand.lookAt(entityIn, source.getEntityAnchorType(), this.entity,
							this.anchor);
