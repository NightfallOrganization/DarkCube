...
public class HandlerCreateSnapshot implements PacketHandler<PacketCreateSnapshot> {
    @Override
    public Packet handle(PacketCreateSnapshot packet) throws ExecutionException, InterruptedException {
        return new Response(PServerProvider.instance().pserver(packet.id()).get().createSnapshot().get());
    }
}
