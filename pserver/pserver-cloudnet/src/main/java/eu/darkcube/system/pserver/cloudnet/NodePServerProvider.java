...
public class NodePServerProvider extends PServerProvider {
    ...
    private NodePServerProvider() {
        ...
        // ServiceTask
        {
            if (!CloudNet.getInstance().getServiceTaskProvider().isServiceTaskPresent(NodePServerProvider.pserverTaskName)) {
                ...
                ProcessConfiguration processConfiguration = new ProcessConfiguration(ServiceEnvironmentType.MINECRAFT_SERVER, 312, new ArrayList<>());
                int startPort = 20000;
                int minServiceCount = 0;

                this.pserverTask = new ServiceTask(includes, templates, deployments, NodePServerProvider.pserverTaskName, runtime, maintenance, autoDeleteOnStop, staticServices, associatedNodes, groups, deletedFilesAfterStop, processConfiguration, startPort, minServiceCount);
                CloudNet.getInstance().getServiceTaskProvider().addPermanentServiceTask(this.pserverTask);
            } else {
                this.pserverTask = CloudNet.getInstance().getServiceTaskProvider().getServiceTask(NodePServerProvider.pserverTaskName);
            }
        }
        // Groups
        {
            if (!CloudNetDriver.getInstance().getGroupConfigurationProvider().isGroupConfigurationPresent("pserver-global")) {
                CloudNetDriver
                        .getInstance()
                        .getGroupConfigurationProvider()
                        .addGroupConfiguration(new EmptyGroupConfiguration("pserver-global"));
            }
            if (!CloudNetDriver.getInstance().getGroupConfigurationProvider().isGroupConfigurationPresent("pserver-world")) {
                CloudNetDriver
                        .getInstance()
                        .getGroupConfigurationProvider()
        ...
        {
            String name = ".global";
            this.globalTemplate = new ServiceTemplate(PServerProvider.templatePrefix(), name, LocalTemplateStorage.LOCAL_TEMPLATE_STORAGE);

            if (!this.storage.has(this.globalTemplate)) {
                this.storage.create(this.globalTemplate);
            }
            name = ".world";
            this.worldTemplate = new ServiceTemplate(PServerProvider.templatePrefix(), name, LocalTemplateStorage.LOCAL_TEMPLATE_STORAGE);
            if (!this.storage.has(this.worldTemplate)) {
                this.storage.create(this.worldTemplate);
            }
        }
        this.pserverData = CloudNetDriver.getInstance().getDatabaseProvider().getDatabase("pserver_data");
    }
    ...
    @Override public @NotNull CompletableFuture<@Nullable NodePServerExecutor> pserver(UniqueId pserver) {
    ...
    @Override public CompletableFuture<PServerExecutor> createPServer(PServerBuilder builder) {
        ...
        executor.execute(() -> {
            ...
            String taskName = builder.taskName() == null ? this.pserverTask.getName() : builder.taskName();
    ...
    @Override public CompletableFuture<Collection<UniqueId>> pservers(UUID owner) {
        return CompletableFuture.completedFuture(DatabaseProvider.get("pserver").cast(PServerDatabase.class).getPServers(owner));
    }
