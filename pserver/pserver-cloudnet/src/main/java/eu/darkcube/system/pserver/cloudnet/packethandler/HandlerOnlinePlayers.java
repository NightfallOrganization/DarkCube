...
public class HandlerOnlinePlayers implements PacketHandler<PacketOnlinePlayers> {
    @Override
    public Packet handle(PacketOnlinePlayers packet) throws ExecutionException, InterruptedException {
        return new Response(PServerProvider.instance().pserver(packet.id()).get().onlinePlayers().get());
    }
}
