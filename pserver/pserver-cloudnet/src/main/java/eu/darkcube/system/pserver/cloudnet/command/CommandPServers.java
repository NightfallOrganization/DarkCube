...
public class CommandPServers extends SubCommandHandler {
	...
	private static void displayInformation(ICommandSender sender, PServerExecutor ps) {
		try {
			sender.sendMessage("* ID: " + ps.id().toString(),
					"* Online: " + ps.onlinePlayers().get(),
					"* Ontime: " + TimeUnit.MILLISECONDS.toSeconds(ps.ontime().get()) + "s",
					"* ServerName: " + ps.serverName().get(),
					"* Owners: " + ps.owners().get().toString(),
					"* State: " + ps.state().get().toString());
		} catch (InterruptedException | ExecutionException e) {
			throw new RuntimeException(e);
		}
	}
