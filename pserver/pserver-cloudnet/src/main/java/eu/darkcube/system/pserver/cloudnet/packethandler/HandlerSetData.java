...
import eu.darkcube.system.pserver.common.packets.PacketWrapperNodeSetData;
...
public class HandlerSetData implements PacketHandler<PacketWrapperNodeSetData> {
	...
	public Packet handle(PacketWrapperNodeSetData packet) {
		try {
			NodePServerProvider.getInstance()
					.setPServerData(packet.getPServerId(), packet.getData()).get();
