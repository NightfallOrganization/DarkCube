...
public sealed interface EventNode<T> permits EventNodeImpl {
    ...
    private static <E, V> EventNode<E> create(@NotNull String name, @NotNull EventFilter<E, V> filter, @Nullable BiPredicate<E, V> predicate) {
    ...
    default void callCancellable(@NotNull T event, @NotNull Runnable successCallback) {
        call(event);
        if (!(event instanceof Cancellable cancellableEvent) || !cancellableEvent.cancelled()) {
            successCallback.run();
        }
    }
