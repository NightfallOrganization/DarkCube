...
public abstract class Command implements Taber {
	...
	public Command(JavaPlugin plugin, String name, Command[] childs, String beschreibung,
		...
		try {
			this.permission =
					"system.plugin." + plugin.getName().toLowerCase().replace(" ", "") + ".command";
	...
	public final void onCommand(CommandSender sender, String[] args, String start) {
		if (hasPermission(sender)) {
			...
			sender.sendMessage(
					VersionSupport.getVersion().commandApi().getSpigotUnknownCommandMessage());
	}
	...
	public final String getUsage(CommandSender sender, String start) {
		if (hasPermission(sender)) {
			...
			if (hasSubCommands()) {
				...
				builder.append(this.instance.prefix).append("§3§lCommand Beschreibung - §b")
						.append(niceCommandName).append('\n').append(this.instance.prefix)
						.append("§b> §7").append(this.beschreibung);
				if (arguments.length != 0) {
					...
					for (Argument argument : arguments) {
						...
						if (argument.getDescription() != null
								&& argument.getDescription().length() != 0) {
		...
		return VersionSupport.getVersion().commandApi().getSpigotUnknownCommandMessage();
	}
		...
		if (hasPermission(sender)) {
			if (hasSubCommands()) {
				...
				if (niceCommandName.length() > 0)
					niceCommandName = Character.toString(niceCommandName.charAt(0)).toUpperCase()
							+ niceCommandName.substring(1);
				usage.append(instance.prefix).append("§b").append(niceCommandName)
						.append(" Sub Commands: ");
						...
						if (cmd.getName().toLowerCase().startsWith(start.toLowerCase())) {
							...
							if (cmd.arguments.length != 0) {
								...
								for (Argument argument : cmd.arguments) {
									if (argument.isNeeded()) {
										usage.append(" §8<§7").append(argument.getName())
												.append("§8>");
									} else {
										usage.append(" §8[§7").append(argument.getName())
												.append("§8]");
	...
	public final boolean hasPermission(CommandSender sender, String permission) {
		...
		for (PermissionAttachmentInfo perminfo : sender.getEffectivePermissions()) {
			if (perminfo.getPermission().startsWith(permission) || perminfo.getPermission()
					.equals("*"))
				return true;
		}
		return false;
