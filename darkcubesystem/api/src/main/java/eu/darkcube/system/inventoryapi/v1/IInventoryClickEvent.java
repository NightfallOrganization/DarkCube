...
public class IInventoryClickEvent extends Event implements Cancellable {
	...
	public IInventoryClickEvent(InventoryClickEvent bukkitEvent, IInventory inventory) {
		...
		this.user = UserAPI.getInstance().getUser(bukkitEvent.getWhoClicked().getUniqueId());
		this.builder = bukkitEvent.getCurrentItem() == null
				? null
				: ItemBuilder.item(bukkitEvent.getCurrentItem());
	}
