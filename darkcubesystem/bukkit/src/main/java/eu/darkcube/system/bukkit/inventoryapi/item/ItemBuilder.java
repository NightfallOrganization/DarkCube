...
@Api public interface ItemBuilder {
    ...
    @Api static ItemBuilder item(Material material) {
        return BukkitVersion.version().itemProvider().item(material);
    }

    @Api static ItemBuilder item(ItemStack item) {
        return BukkitVersion.version().itemProvider().item(item);
    }

    @Api static ItemBuilder item(JsonElement json) {
        return BukkitVersion.version().itemProvider().item(json);
    }

    @Api static ItemBuilder spawner() {
        return BukkitVersion.version().itemProvider().spawner();
    }
