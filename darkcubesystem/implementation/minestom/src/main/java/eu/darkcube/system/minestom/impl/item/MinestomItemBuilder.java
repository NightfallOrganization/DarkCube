...
import eu.darkcube.system.minestom.impl.item.attribute.MinestomAttributeModifier;
import eu.darkcube.system.minestom.impl.item.enchant.MinestomEnchantment;
import eu.darkcube.system.minestom.impl.item.firework.MinestomFireworkEffect;
import eu.darkcube.system.minestom.impl.item.flag.MinestomItemFlag;
import eu.darkcube.system.minestom.impl.item.material.MinestomMaterial;
...
public class MinestomItemBuilder extends AbstractItemBuilder {
    ...
    public MinestomItemBuilder() {
    }

    public MinestomItemBuilder(ItemStack item) {
        var meta = item.meta();
    ...
    public static MinestomItemBuilder deserialize(JsonElement json) {
        return new MinestomItemBuilder(gson.fromJson(json, ItemStack.class));
    }

    @Override public @NotNull ItemStack build() {
        var material = ((MinestomMaterial) this.material).minestomType();
        ...
        builder.meta(meta -> {
            ...
            for (var entry : enchantments.entrySet()) {
                meta.enchantment(((MinestomEnchantment) entry.getKey()).minestomType(), entry.getValue().shortValue());
            }

            meta.hideFlag(flags.stream().map(flag -> ((MinestomItemFlag) flag).minestomType()).toArray(ItemHideFlag[]::new));
            ...
            attributeModifiers.forEach((attribute, modifiers) -> meta.attributes(modifiers.stream().map(modifier -> ((MinestomAttributeModifier) modifier).minestomType()).toList()));
            ...
            for (var builderMeta : metas) {
                switch (builderMeta) {
                    case FireworkBuilderMeta fireworkBuilderMeta -> builder.meta(FireworkMeta.class, m -> {
                        var cast = ((MinestomFireworkEffect) fireworkBuilderMeta.fireworkEffect());
                        if (cast != null) {
                    ...
                    case EnchantmentStorageBuilderMeta enchantmentStorageBuilderMeta -> builder.meta(EnchantedBookMeta.class, m -> {
                        for (var entry : enchantmentStorageBuilderMeta.enchantments().entrySet()) {
                            m.enchantment(((MinestomEnchantment) entry.getKey()).minestomType(), entry.getValue().shortValue());
                        }
                    });
    ...
    @Override public @NotNull MinestomItemBuilder clone() {
        return new MinestomItemBuilder(build());
    }
