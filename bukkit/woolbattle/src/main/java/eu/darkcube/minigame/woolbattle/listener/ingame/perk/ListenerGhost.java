...
public class ListenerGhost extends BasicPerkListener {
	...
	private static final String METADATA_GHOSTPOS = "ghostPos";
	private static final String METADATA_GHOSTATTACKS = "ghostAttacks";
	...
	public static void reset(User user) {
		reset0(user, user.getTemporaryDataStorage().get(METADATA_GHOSTPOS));
	}

	public static boolean isGhost(User user) {
		return user.getTemporaryDataStorage().has(METADATA_GHOSTPOS);
	}
	...
	protected boolean activateRight(User user, Perk perk) {
		...
		user.getTemporaryDataStorage().set(METADATA_GHOSTPOS, p.getLocation());
	...
	public void handle(PlayerQuitEvent e) {
		...
		user.getTemporaryDataStorage().remove(METADATA_GHOSTPOS);
		user.getTemporaryDataStorage().remove(METADATA_GHOSTATTACKS);
	}
			...
			if (isGhost(user)) {
				...
				if (p.getNoDamageTicks() == 0) {
					if (WoolBattle.getInstance().getIngame().attack(attacker, user, true)) {
						if (!user.getTemporaryDataStorage().has(METADATA_GHOSTATTACKS)) {
							user.getTemporaryDataStorage().set(METADATA_GHOSTATTACKS, 1);
