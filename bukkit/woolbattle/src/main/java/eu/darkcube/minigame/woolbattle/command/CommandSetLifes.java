	...
	public CommandSetLifes() {
		super("woolbattle", "setlifes", "woolbattle.command.setlifes",
						...
						b -> b.then(Commands.argument("lifes", IntegerArgumentType.integer(0, 99)).executes(context -> {
							Main.getInstance().baseLifes = IntegerArgumentType.getInteger(context, "lifes");
							context.getSource().sendFeedback(CustomComponentBuilder.cast(TextComponent.fromLegacyText(Message.CHANGED_LIFES.getServerMessage(Integer.toString(Main.getInstance().baseLifes)))), true);
							return 0;
							...
							Team team = Main.getInstance().getTeamManager().getTeam(TeamArgument.getTeam(context, "team"));
