...
public class BlinkPerk extends Perk {
	...
	public BlinkPerk() {
		...
		addListener(new ListenerBlink(this));
	}
	...
	public static class ListenerBlink extends BasicPerkListener {
		...
		private static boolean teleport(WBUser user) {
			...
			int dist = 15;
			if (p.isSneaking()) {
				RayTrace raytrace = new RayTrace(p.getEyeLocation().toVector(),
				...
				for (Vector position : positions) {
					...
					if (!ListenerBlink.checkBlock(pos)) {
						if (ListenerBlink.checkBlock(pos.add(0, 1, 0)) && ListenerBlink.checkBlock(
								pos.add(0, 1, 0))) {
			...
			for (; dist > 1; dist--) {
				Location loc =
						user.getBukkitEntity().getEyeLocation().add(v.clone().multiply(dist));
				...
				if (loc.getBlock().getType() == Material.AIR
					...
					if (free) {
						...
						user.getBukkitEntity().teleport(
								loc.getBlock().getLocation().add(.5, .1, .5).setDirection(v));
					}
					return true;
				}
			}
