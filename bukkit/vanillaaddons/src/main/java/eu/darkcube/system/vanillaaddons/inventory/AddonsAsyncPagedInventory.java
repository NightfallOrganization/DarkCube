...
public class AddonsAsyncPagedInventory extends AsyncPagedInventory {
    ...
    public AddonsAsyncPagedInventory(InventoryType inventoryType, Component title, BooleanSupplier instant) {
        this(inventoryType, title, 5 * 9, AsyncPagedInventory.box(2, 2, 4, 8), instant);
    }

    public AddonsAsyncPagedInventory(InventoryType inventoryType, Component title, int size, int[] box, BooleanSupplier instant) {
        super(inventoryType, title, size, instant, box, IInventory.slot(1, 5));
    }

    @Override protected void postTick(boolean changedInformations) {
        if (changedInformations) {
            this.playSound();
        }
    }

    @Override protected void insertDefaultItems() {
...
//				.forEach(p -> p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BANJO, 100, 1));
    ...
    protected void insertArrowItems() {
        this.arrowSlots.putIfAbsent(PageArrow.PREVIOUS, new Integer[]{slot(3, 1), slot(4, 1), slot(5, 1)});
        this.arrowSlots.putIfAbsent(PageArrow.NEXT, new Integer[]{slot(3, 9), slot(4, 9), slot(5, 9)});
    }

    protected void insertFallbackItems() {
        ItemStack l = ItemBuilder
                .item(Material.BROWN_STAINED_GLASS_PANE)
