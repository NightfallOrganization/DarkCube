...
public class MessagingModule implements Module {
    ...
    public MessagingModule(VanillaAddons addons) {
        this.addons = addons;
        this.key = Key.key(addons, "command_r_last_message_by");
    ...
    private void message(CommandSource source, Collection<Player> targets, String message) throws CommandSyntaxException {
        ...
        for (Player target : targets) {
            ...
            user.sendMessage(MESSAGE, source.getName(), PlainTextComponentSerializer.plainText().serialize(SELF_RECEIVED.getMessage(user)), message);
            target.playSound(target.getLocation(), Sound.BLOCK_NOTE_BLOCK_HAT, 0.5F, 2.0F);
            source.sendMessage(MESSAGE, PlainTextComponentSerializer.plainText().serialize(SELF_SENT.getMessage(source.getSource())), target.getName(), message);
            if (sender != null) {
    ...
    private final class CommandMessage extends Command {
        ...
        public CommandMessage() {
            super("vanillaaddons", "message", new String[]{"msg"}, b -> b.then(Commands.argument("targets", EntityArgument.players()).then(Commands.argument("message", StringArgumentType.greedyString()).executes(ctx -> {
    ...
    private final class CommandReply extends Command {
        ...
        public CommandReply() {
            super("vanillaaddons", "reply", new String[]{"r"}, b -> b.then(Commands.argument("message", StringArgumentType.greedyString()).executes(ctx -> {
