...
public class DeathChestsModule implements Module, Listener {
	private static final List<Vector> RELATIVE_POSTIONS =
			Arrays.asList(new Vector(1, 0, 0), new Vector(1, 0, 1), new Vector(0, 0, 1),
					new Vector(-1, 0, 0), new Vector(-1, 0, -1), new Vector(0, 0, -1),
					new Vector(-1, 0, 1), new Vector(1, 0, -1), new Vector(1, 1, 0),
					new Vector(1, 1, 1), new Vector(0, 1, 1), new Vector(-1, 1, 0),
					new Vector(-1, 1, -1), new Vector(0, 1, -1), new Vector(-1, 1, 1),
					new Vector(1, 1, -1), new Vector(0, 1, 0), new Vector(1, -1, 0),
					new Vector(1, -1, 1), new Vector(0, -1, 1), new Vector(-1, -1, 0),
					new Vector(-1, -1, -1), new Vector(0, -1, -1), new Vector(-1, -1, 1),
	...
	public void onDeath(PlayerDeathEvent e) {
		if (isPlayerKill(e.getPlayer())) {
			...
			if (Math.random() < .5)
				e.getDrops()
						.add(getPlayerHead(e.getEntity().getName(), e.getEntity().getUniqueId()));
			return;
		}

		if (Math.random() < .5)
			e.getDrops().add(getPlayerHead(e.getEntity().getName(), e.getEntity().getUniqueId()));

		ItemStack expb = new ItemStack(Material.EXPERIENCE_BOTTLE,
				(int) ((e.getEntity().getTotalExperience() / 7.0) * .66));
		if (expb.getAmount() > 64)
			expb.setAmount(64);
	...
	public void onDamage(EntityDamageEvent e) {
		...
		if (inv.getItemInMainHand().getType().equals(Material.TOTEM_OF_UNDYING)
				|| inv.getItemInOffHand().getType().equals(Material.TOTEM_OF_UNDYING))
			return;
	...
	private ItemStack getPlayerHead(String name, UUID uuid) {
		ItemBuilder item = ItemBuilder.item(Material.PLAYER_HEAD)
				.meta(new SkullBuilderMeta(new UserProfile(name, uuid)))
				.displayname(Component.text(name, NamedTextColor.YELLOW));
		return item.build();
	}

	private Location getSafeLocation(Location start) {
		if (start.getBlock().getType().equals(Material.AIR))
			return start;
		for (Vector v : RELATIVE_POSTIONS) {
			if (start.clone().add(v).getBlock().getType().equals(Material.AIR))
				return start.clone().add(v);
		}
		return null;
	}
