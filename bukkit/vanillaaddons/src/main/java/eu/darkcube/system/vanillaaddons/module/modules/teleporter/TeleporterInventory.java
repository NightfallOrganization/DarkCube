		...
		AddonsAsyncPagedInventory i =
				new AddonsAsyncPagedInventory(TYPE, data().dname(), () -> true) {
					...
					protected void inventoryClick(IInventoryClickEvent event) {
						if (Objects.equals(event.bukkitEvent().getClickedInventory(),
								event.bukkitEvent().getView().getTopInventory())) {
							event.setCancelled(true);
						}
						ItemStack bitem = event.bukkitEvent().getView()
								.getItem(event.bukkitEvent().getRawSlot());
						ItemBuilder item = bitem == null ? null : ItemBuilder.item(bitem);
						if (item == null)
							return;
						if (!item.persistentDataStorage().has(KEY_TYPE))
							return;
						int type = item.persistentDataStorage()
								.get(KEY_TYPE, PersistentDataTypes.INTEGER);
						if (type == 0) {
							//noinspection DataFlowIssue
							if (event.bukkitEvent().getCursor() == null
									|| event.bukkitEvent().getCursor().getItemMeta() == null)
								return;
							if (data().owner() == null)
							...
							data().access(TeleportAccess.values()[(data().access().ordinal() + 1)
									% TeleportAccess.values().length]);
							insertFallbackItems();
					...
					protected void insertFallbackItems() {
						ItemStack icon = ItemBuilder.item(teleporter.icon()).displayname(
						...
						ItemStack name = ItemBuilder.item(Material.NAME_TAG).displayname(
										Component.text("Name").color(TextColor.color(120, 120,
												120)))
								.lore(Component.join(JoinConfiguration.separator(Component.space()),
										Component.text("Name:")
												.color(TextColor.color(120, 120, 120)),
										data().dname())).persistentDataStorage()
								.iset(KEY_TYPE, PersistentDataTypes.INTEGER, 1).builder().build();
						ItemStack access = ItemBuilder.item(teleporter.access().getType())
								.displayname(Component.join(
										JoinConfiguration.separator(Component.space()),
										Component.text("Zugriff:")
												.color(TextColor.color(120, 120, 120)),
										Component.text(teleporter.access().name().toLowerCase())
												.color(TextColor.color(170, 0, 170))))
								.persistentDataStorage()
								.iset(KEY_TYPE, PersistentDataTypes.INTEGER, 2).builder().build();
