...
import eu.darkcube.system.bukkit.inventoryapi.item.meta.EnchantmentStorageBuilderMeta;
...
public class AnvilMechanicsModule implements Module, Listener {
    ...
    @EventHandler(priority = EventPriority.LOWEST) public void handle(PrepareAnvilEvent event) {
        ...
        ItemBuilder first = event.getInventory().getFirstItem() == null ? null : ItemBuilder.item(event.getInventory().getFirstItem());
        ItemBuilder second = event.getInventory().getSecondItem() == null ? null : ItemBuilder.item(event.getInventory().getSecondItem());
        ItemBuilder result = event.getResult() == null ? null : ItemBuilder.item(event.getResult());
        Data data = simulateAnvil(event.getView().getPlayer(), first, second, result, event.getInventory().getRenameText(), event
                .getInventory()
    ...
    private Data simulateAnvil(HumanEntity player, ItemBuilder first, ItemBuilder second, ItemBuilder result, String renameText, int levelCost, int itemCost, int maximumRepairCost) {
        ...
        if (second != null) {
            boolean enchantedBook = second.material() == Material.ENCHANTED_BOOK && !second
            ...
            if (result.material().getMaxDurability() != 0 && first.canBeRepairedBy(second)) {
                ...
                if (!enchantedBook && (result.material() != second.material() || result.material().getMaxDurability() == 0)) {
                    return originalData;
                }
                ...
                for (Enchantment enchantment : secondEnchantments.keySet()) {
                    {
                        ...
                        if (player.getGameMode() == GameMode.CREATIVE || first.material() == Material.ENCHANTED_BOOK) {
                            canEnchantItem1 = true;
                        }

                        for (Enchantment enchantment1 : resultEnchantments.keySet()) {
                            if (!enchantment1.equals(enchantment) && enchantment.conflictsWith(enchantment1)) {
        ...
        } else if (first.displayname() == null || !renameText.equals(LegacyComponentSerializer
                .legacySection()
    ...
    private Map<Enchantment, Integer> enchantments(ItemBuilder item) {
        return new HashMap<>(item.material() == Material.ENCHANTED_BOOK ? item
                .meta(EnchantmentStorageBuilderMeta.class)
