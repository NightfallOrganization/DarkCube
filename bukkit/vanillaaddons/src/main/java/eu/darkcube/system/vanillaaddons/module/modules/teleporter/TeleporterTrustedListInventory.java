...
public class TeleporterTrustedListInventory
		extends AbstractInventory<AddonsAsyncPagedInventory, Teleporter> {
	...
	protected AddonsAsyncPagedInventory openInventory(AUser user) {
		...
		return new AddonsAsyncPagedInventory(TYPE, data().dname(), () -> true) {
			...
			protected void inventoryClick(IInventoryClickEvent event) {
				...
				if (type == 0) {
					AUser.user(event.user())
							.openInventory(TeleporterTrustedListAddInventory.TYPE, data());
				} else if (type == 1) {
			...
			protected void fillItems(Map<Integer, ItemStack> items) {
				...
				for (UUID uuid : data().trustedList()) {
					...
					String name = op.hasPlayedBefore() ? op.getName() == null
							? "Unbekannter Spieler"
			...
			protected void insertFallbackItems() {
				fallbackItems.put(IInventory.slot(1, 5), ItemBuilder.item(Material.EMERALD)
