    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (player.getLocation().getY() <= 90) {
            if (teamManager.isPlayerInTeam(player)) {
                ...
                Location spawnLocation = mapTeamSpawns.getSpawnLocation("WBT-1", team.getName());

                if (spawnLocation != null) {
                    player.teleport(spawnLocation);
                } else {
                    player.teleport(new Location(player.getWorld(), 0, 140, 0));
                }
            } else {
                player.teleport(new Location(player.getWorld(), 0, 140, 0));
            }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        if (event.getWhoClicked() instanceof Player) {
            ...
            if (!world.getName().equalsIgnoreCase("world")) {
                ...
                // Wenn das angeklickte Item nicht null und nicht Wolle ist
