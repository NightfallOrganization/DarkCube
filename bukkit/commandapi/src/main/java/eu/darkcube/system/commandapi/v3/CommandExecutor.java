...
public class CommandExecutor {
	...
	public void register(CommandDispatcher<CommandSource> dispatcher) {
		...
		this.argumentBuilder.accept(builder);
		LiteralCommandNode<CommandSource> node = dispatcher.register(builder);

		for (String name : getNames()) {
			dispatcher.register(buildRedirect(name, node));
			dispatcher.register(buildRedirect(prefix + ":" + name, node));
		}
	}

	protected LiteralArgumentBuilder<CommandSource> buildRedirect(final String alias,
		...
		LiteralArgumentBuilder<CommandSource> builder =
				Commands.literal(alias.toLowerCase(Locale.ENGLISH))
						.requires(destination.getRequirement()).forward(destination.getRedirect(),
								destination.getRedirectModifier(), destination.isFork())
