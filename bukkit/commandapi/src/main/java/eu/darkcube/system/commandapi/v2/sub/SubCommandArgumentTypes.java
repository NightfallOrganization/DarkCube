...
public class SubCommandArgumentTypes {
	...
	public static QuestionAnswerType<String> url(String key) {
		return dynamicString(key, "Invalid url!", input -> {
			try {
				new URL(input);
				return true;
	...
	public static QuestionAnswerType<String> dynamicString(String key, int maxLength) {
		return dynamicString(key, "String is too long!".replace("%maxLength%", String.valueOf(maxLength)),
				value -> value.length() <= maxLength);
	}

	public static QuestionAnswerType<String> dynamicString(String key, String invalidMessage,
	...
	public static QuestionAnswerType<Double> double_(String key, String invalidMessage, Predicate<Double> tester,
		...
		return new QuestionAnswerTypeDouble() {
			...
			@Override
			public Collection<String> getPossibleAnswers() {
				return possibleAnswersSupplier != null
						? possibleAnswersSupplier.get().stream().map(Objects::toString).collect(Collectors.toList())
						: null;
			}

			@Override
			public boolean isValidInput(String input) {
				return super.isValidInput(input) && (tester == null || tester.test(Double.parseDouble(input)));
			}
		};
	}

	public static QuestionAnswerType<Integer> positiveInteger(String key) {
		return integer(key, "Integer is not positive!", value -> value > 0);
	}

	public static QuestionAnswerType<Integer> negativeInteger(String key) {
		return integer(key, "Integer is not negative!", value -> value < 0);
	}
