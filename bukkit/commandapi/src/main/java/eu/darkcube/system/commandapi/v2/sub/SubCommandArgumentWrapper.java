...
public class SubCommandArgumentWrapper {
	...
	public Optional<QuestionAnswerType<?>> argumentType(String key) {
		return this.argumentStream(key).findFirst().map(SubCommandArgument::getAnswerType);
	}

	public Optional<Object> argument(String key) {
		return this.argumentStream(key).findFirst().map(SubCommandArgument::getAnswer);
	}

	public Optional<Object> argument(
			@SuppressWarnings("rawtypes") Class<? extends QuestionAnswerType> answerTypeClass) {
		return Arrays.stream(this.arguments)
