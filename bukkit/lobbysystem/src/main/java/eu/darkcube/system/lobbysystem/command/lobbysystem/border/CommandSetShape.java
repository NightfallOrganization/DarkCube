...
public class CommandSetShape extends LobbyCommand {

    public CommandSetShape() {
        super("setShape", b -> b.then(Commands.argument("shape", EnumArgument.enumArgument(Shape.values())).executes(ctx -> {
            ...
            ctx.getSource().sendMessage(Component.text("Form neugesetzt!").color(NamedTextColor.GREEN));
            return 0;
        })));
    }
