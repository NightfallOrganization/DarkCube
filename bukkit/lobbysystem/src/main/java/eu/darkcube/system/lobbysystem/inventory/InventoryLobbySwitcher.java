...
//	};
    ...
    @Override protected void fillItems(Map<Integer, ItemStack> items) {
        ...
        ServiceInfoSnapshot serviceInfo = InjectionLayer.boot().instance(ServiceInfoHolder.class).serviceInfo();
        ServiceId service = serviceInfo.serviceId();
        String taskName = service.taskName();
        List<ServiceInfoSnapshot> lobbies = InjectionLayer
                .boot()
                .instance(CloudServiceProvider.class)
                .servicesByTask(taskName)
                .stream()
                .filter(ServiceInfoSnapshot::connected)
                .sorted(Comparator.comparingInt(o -> o.serviceId().taskServiceId()))
                .toList();
        ...
        for (int i = 0; i < Math.min(InventoryLobbySwitcher.SLOTS.length, lobbyCount); i++) {
            ...
            item = ItemBuilder
                    ...
                    .iset(server, PersistentDataTypes.STRING, s.serviceId().name())
                    .builder()
                    .build();
            items.put(i, item);
        }
