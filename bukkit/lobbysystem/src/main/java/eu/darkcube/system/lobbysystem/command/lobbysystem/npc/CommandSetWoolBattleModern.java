...
public class CommandSetWoolBattleModern extends LobbyCommand {

    public CommandSetWoolBattleModern() {
        super("setWoolBattleModern", b -> {
    ...
    private static int setSpawn(CommandContext<CommandSource> context, ILocationArgument location, ILocationArgument rotation, boolean makenice) {
        ...
        Lobby.getInstance().getDataManager().setWoolBattleModernNPCLocation(loc);
        context.getSource().sendMessage(Component.text("NPC neugesetzt!").color(NamedTextColor.GREEN));
        return 0;
    }
