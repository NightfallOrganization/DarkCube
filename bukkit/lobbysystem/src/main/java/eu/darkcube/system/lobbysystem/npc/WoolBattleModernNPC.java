/*
 * Copyright (c) 2022-2024. [DarkCube]
 * All rights reserved.
 * You may not use or redistribute this software or any associated files without permission.
 * The above copyright notice shall be included in all copies of this software.
 */

package eu.darkcube.system.lobbysystem.npc;

import eu.darkcube.system.lobbysystem.Lobby;

public class WoolBattleModernNPC {

    public static NPCManagement.NPC create() {
        NPCManagement.NPC npc = Lobby.getInstance().npcManagement().builder().profileHelper().name("§5W§dB §6Modern").skin(new WoolBattleSkin()).builder().build();
        npc.location(Lobby.getInstance().getDataManager().getWoolBattleModernNPCLocation());

        new NPCKnockbackThread(npc).runTaskTimer(Lobby.getInstance(), 5, 5);
        return npc;
    }

    public static class WoolBattleSkin extends NPCManagement.Skin {

        public WoolBattleSkin() {
            super("",
                    "");
        }

    }

}
