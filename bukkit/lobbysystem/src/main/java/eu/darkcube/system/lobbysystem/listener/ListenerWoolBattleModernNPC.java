...
import eu.darkcube.system.lobbysystem.inventory.InventoryWoolBattleModern;
...
public class ListenerWoolBattleModernNPC extends BaseListener {

    @EventHandler
    public void handle(PlayerNPCInteractEvent e) {
        NPCManagement.NPC npc = e.npc();
        if (npc.equals(Lobby.getInstance().getWoolBattleModernNPC())) {
            if (e.useAction() == PlayerNPCInteractEvent.EntityUseAction.ATTACK) {
                ...
                user.setOpenInventory(new InventoryWoolBattleModern(user.user()));
            }
        }
    }
