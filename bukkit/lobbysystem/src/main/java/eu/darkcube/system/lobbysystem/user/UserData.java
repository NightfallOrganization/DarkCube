...
public class UserData implements Serializable {
    ...
    public UserData(UUID owner, Document document) {
        if (document == null) {
        ...
        if (document.contains("language")) {
            ...
            UserAPI.getInstance().getUser(owner).language(language);
        }
        ...
        rewardSlotsUsed = document.readObject("rewardSlotsUsed", new TypeToken<Set<Integer>>() {
            private static final long serialVersionUID = -2586151616370404958L;
        }.getType(), rewardSlotsUsed);
        if (rewardSlotsUsed == null) rewardSlotsUsed = new HashSet<>();
    }

    public UserData(Language language, Gadget gadget, boolean sounds, boolean animations, long lastDailyReward, Set<Integer> rewardSlotsUsed) {
