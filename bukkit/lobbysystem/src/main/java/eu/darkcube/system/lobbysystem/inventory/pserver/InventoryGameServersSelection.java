...
public class InventoryGameServersSelection extends LobbyAsyncPagedInventory {
    ...
    public InventoryGameServersSelection(User user) {
        super(InventoryGameServersSelection.type_gameserver_selection, Item.GAME_PSERVER.getDisplayName(user), user);
    }
    ...
    @Override protected void inventoryClick(IInventoryClickEvent event) {
        ...
        if (itemid.equals(Item.GAMESERVER_SELECTION_WOOLBATTLE.getItemId())) {
            user.setOpenInventory(new InventoryGameServerSelectionWoolBattle(user.user()));
        }
    }

    @Override protected void fillItems(Map<Integer, ItemStack> items) {
        super.fillItems(items);
        items.put(this.getPageSize() / 2, Item.GAMESERVER_SELECTION_WOOLBATTLE.getItem(this.user.user()));
    }

    @Override protected void insertFallbackItems() {
        this.fallbackItems.put(IInventory.slot(1, 5), Item.GAME_PSERVER.getItem(this.user.user()));
