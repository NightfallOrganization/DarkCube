...
public class GamemodeConnectorArgument implements ArgumentType<ConnectorNPC> {
    private static final DynamicCommandExceptionType NPC_NOT_FOUND = Message.CONNECTOR_NPC_NOT_FOUND.newDynamicCommandExceptionType();

    @Override public ConnectorNPC parse(StringReader stringReader) throws CommandSyntaxException {
        var key = stringReader.readString();
        var npc = ConnectorNPC.get(key);
        if (npc == null) throw NPC_NOT_FOUND.createWithContext(stringReader, key);
        return npc;
    }
