...
public class JaR {
    ...
    public JaR(JaRManager manager, LobbyUser user) {
        ...
        (particles = new BukkitRunnable() {
            @Override public void run() {
                ...
                if (node == null) return;
                ParticleEffect.VILLAGER_HAPPY.display(0.2F, 0.2F, 0.2F, 1, 1, getCurrentBlock()
                        .getLocation()
                        .add(0.5, 1.5, 0.5), user.asPlayer());
                ...
                user
                        ...
                        .sendActionBar(Component
                                ...
                                .append(Component.text(Integer.toString(n)).color(NamedTextColor.LIGHT_PURPLE))
    ...
    public void reset() {
        if (node != null) deleteAll(node);
        oldNode = null;
        fallbackNode = node;
        node = null;
        generate();
        if (getCurrentBlock() != null)
            user.asPlayer().teleport(getCurrentBlock().getLocation().add(0.5, 1, 0.5).setDirection(getCurrentDirection()));
    ...
    private void generate() {
        if (destroyed) return;
        if (node == null) {
            if (manager.regions.isEmpty()) return;
            JaRRegion region = oldNode != null ? oldNode.region : manager.regions.get(r.nextInt(manager.regions.size()));
            Vector start = new Vector(region.x, region.y, region.z).add(new Vector(r.nextInt(region.widthX), r.nextInt(region.height / 5), r.nextInt(region.widthZ)));
        ...
        while (depth < PREGEN) {
            ...
            Vector center = new Vector(current.region.x + current.region.widthX / 2D, current.region.y + current.region.height / 2D, current.region.z + current.region.widthZ / 2D);
            Vector forcedDir = center.subtract(current.block);
            forcedDir.setY(0);
            forcedDir.normalize();
            double distX = distance(current.block.getX(), current.region.x, current.region.x + current.region.widthX);
            double distZ = distance(current.block.getZ(), current.region.z, current.region.z + current.region.widthZ);
            ...
            boolean usingFactors = ((factorX != 0 || factorZ != 0) && dot < 0.3) || (distX < 15 || distZ < 15);
            if (usingFactors) turn = current.turn;
            cross.multiply(turn);
            if (usingFactors) {
            ...
            if (current.block.getY() > current.region.y + current.region.height - 5 && current.count % 5 == 0) {
                current.last = true;
                build(current);
                return;
            }
    ...
    private double calculateFactorDist(double dist) {
        ...
        return 1 - Math.pow(Math.min(1, Math.max(0, (dist - FORCE_FIELD_MIN_DISTANCE)) / (FORCE_FIELD_MAX_DISTANCE)), FORCE_FIELD_STRENGTH);
    }
    ...
    @SuppressWarnings("deprecation") private void build(JarNode node) {
    ...
    private Vector getDirection(JarNode node) {
        ...
        Vector vec = node.block
                .clone()
                .subtract(new Vector(node.region.x + node.region.widthX / 2D, node.region.y + node.region.height / 2D, node.region.z + node.region.widthZ / 2D));
    ...
    private Vector getJump(Vector start, Vector dirIn) {
        start = start.clone().setX(start.getBlockX()).setZ(start.getBlockZ()).add(new Vector(0.5, 0, 0.5));
