...
public class CommandSetPos1 extends LobbyCommandExecutor {
	...
	public CommandSetPos1() {
		super("setPos1", b -> b.executes(ctx -> cmd(ctx, ctx.getSource().asPlayer().getLocation()))
				.then(Commands.argument("makenice", BooleanArgument.booleanArgument()).executes(
						ctx -> cmd(ctx, BooleanArgument.getBoolean(ctx, "makenice")
	...
	private static int cmd(CommandContext<CommandSource> ctx, Location loc) {
		...
		ctx.getSource()
				.sendMessage(Component.text("Position 1 gesetzt!").color(NamedTextColor.GREEN));
		return 0;
	}
