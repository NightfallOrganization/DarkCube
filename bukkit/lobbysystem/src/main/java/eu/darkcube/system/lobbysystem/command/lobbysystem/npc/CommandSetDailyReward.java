...
public class CommandSetDailyReward extends LobbyCommandExecutor {

	public CommandSetDailyReward() {
		super("setDailyReward", b -> {
	...
	private static int setSpawn(CommandContext<CommandSource> context, ILocationArgument location,
		...
		Lobby.getInstance().getDataManager().setDailyRewardNPCLocation(loc);
		context.getSource()
				.sendMessage(Component.text("NPC neugesetzt!").color(NamedTextColor.GREEN));
		return 0;
	}
