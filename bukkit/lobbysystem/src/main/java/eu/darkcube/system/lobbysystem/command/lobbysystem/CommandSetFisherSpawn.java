...
public class CommandSetFisherSpawn extends LobbyCommandExecutor {

	public CommandSetFisherSpawn() {
		super("setFisherSpawn", b -> {
	...
	private static int setSpawn(CommandContext<CommandSource> context, ILocationArgument location,
		...
		Lobby.getInstance().getDataManager().setFisherSpawn(loc);
		context.getSource()
				.sendMessage(Component.text("Spawn neugesetzt!").color(NamedTextColor.GREEN));
		return 0;
	}
