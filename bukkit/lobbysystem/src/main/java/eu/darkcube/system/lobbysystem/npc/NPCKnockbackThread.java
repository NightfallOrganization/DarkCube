...
public class NPCKnockbackThread extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (loc == null) return;
        loc.getWorld().getNearbyEntities(loc.clone().add(0, 0.6, 0), 0.3, 0.7, 0.3).stream().filter(Player.class::isInstance).map(Player.class::cast).filter(npc::shownFor).filter(e -> !e.hasPermission("system.npc.knockback.ignore")).forEach(p -> {
            if (p.isFlying())
                return;
            ...
            npc.animation(p, EntityAnimation.SWING_MAIN_ARM);
            npc.sendEmotes(p, Emotes.KARATE.getId());
        });
    }
}
