    ...
    public CommandAddTask() {
        super("addTask", b -> b.then(Commands.argument("task", new ServiceTaskArgument() {
            ...
            protected Stream<String> tasksStream() {
                return super.tasksStream().filter(t -> !Lobby.getInstance().getDataManager().getWoolBattleModernTasks().contains(t));
            }
        }).executes(ctx -> {
            ServiceTask task = ServiceTaskArgument.getServiceTask(ctx, "task");
            Set<String> tasks = new HashSet<>(Lobby.getInstance().getDataManager().getWoolBattleModernTasks());
