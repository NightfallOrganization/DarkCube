...
public class InventorySettings extends LobbyAsyncPagedInventory {
	...
	protected void insertFallbackItems() {
		this.fallbackItems.put(IInventory.slot(1, 5),
				Item.INVENTORY_SETTINGS.getItem(this.user.getUser()));
		this.fallbackItems.put(IInventory.slot(4, 3),
				this.user.isSounds()
						? Item.INVENTORY_SETTINGS_SOUNDS_ON.getItem(this.user.getUser())
						: Item.INVENTORY_SETTINGS_SOUNDS_OFF.getItem(this.user.getUser()));

		String textureId = "";
		String name = "";
		switch (this.user.getUser().getLanguage()) {
			case ENGLISH:
				textureId =
		...
		this.fallbackItems.put(IInventory.slot(4, 7),
				this.user.isAnimations()
						? Item.INVENTORY_SETTINGS_ANIMATIONS_ON.getItem(this.user.getUser())
						: Item.INVENTORY_SETTINGS_ANIMATIONS_OFF.getItem(this.user.getUser()));
		super.insertFallbackItems();
		// if (user.isAnimations()) {
		// InventorySettings.this.handle.setItem(33,
		// Item.INVENTORY_SETTINGS_ANIMATIONS_ON.getItem(user));
		// } else {
		// InventorySettings.this.handle.setItem(33,
		// Item.INVENTORY_SETTINGS_ANIMATIONS_OFF.getItem(user));
		// }
	}
