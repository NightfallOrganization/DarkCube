...
public class CommandTimer extends CommandExecutor {
	...
	public CommandTimer() {
		super("miners", "timer", new String[0], b -> b.then(
				Commands.argument("time", IntegerArgumentType.integer(1)).executes(context -> {
		...
		switch (Miners.getGamephase()) {
			case 0:
				if (!Miners.getLobbyPhase().getTimer().isRunning())
					Miners.getLobbyPhase().getTimer().start(secs * 1000L);
				else
					Miners.getLobbyPhase().getTimer()
							.setEndTime(System.currentTimeMillis() + secs * 1000L);
				break;
			case 1:
				if (Miners.getMiningPhase().getTimer().isRunning())
					Miners.getMiningPhase().getTimer()
							.setEndTime(System.currentTimeMillis() + secs * 1000L);
				break;
			case 2:
				Bukkit.getWorld(Miners.PVP_WORLD_NAME).getWorldBorder().setSize(0, secs);
				if (Miners.getPVPPhase().getTimer().isRunning())
					Miners.getPVPPhase().getTimer()
							.setEndTime(System.currentTimeMillis() + secs * 1000L);
				break;
			default:
				break;
