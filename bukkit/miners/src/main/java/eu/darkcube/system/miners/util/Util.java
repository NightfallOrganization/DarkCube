...
public class Util {
    ...
    public static void sendActionbar(String message, Player... players) {
        if (message == null)
            return;
        ChatUtils.ChatEntry.buildArray(new ChatUtils.ChatEntry.Builder().text(message).build()).sendPlayer(players);
    }
    ...
    public static void sendActionbarToAll(String message, Player... exclude) {
        ...
        sendActionbar(message, Bukkit.getOnlinePlayers().stream().filter(p -> !excludes.contains(p)).toArray(i -> new Player[i]));
    }
