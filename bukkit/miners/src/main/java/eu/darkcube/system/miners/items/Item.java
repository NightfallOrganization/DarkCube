...
public enum Item {
    ...
    public static final String ITEM_PREFIX = "MINERS_ITEM_";
    ...
    public ItemStack getItem(Language lang, int amount) {
        ...
        Component lore = lang.getMessage(KEY_LORE);
        if (lore.children().isEmpty())
            itemBuilder.lore(lore);
        else
            itemBuilder.lore(lore.children());
    ...
    public ItemStack getItem(Player player, int amount) {
        return getItem(UserAPI.getInstance().getUser(player).getLanguage(), amount);
    }
