...
public class CommandStatsTop extends Command {
    ...
    private static final String[] availableCategoriesWoolbattle = {"elo", "kills", "deaths", "wins", "losses", "kd", "wl"};
    ...
    private CommandStatsTop() {
        super(StatsPlugin.getInstance(), "statstop", new Command[0], "Ruft die Top-Stats ab", new Argument("Spielmodus", "Der Spielmodus"), new Argument("Kategorie", "Die Kategorie"), new Argument("Duration", "Die Duration", false));
    }
    ...
    public static Component getTopPlayers(String tableBaseName, String category, int playerCount, String command, Duration duration) {
        Result result = MySQL.mysql.getResult("SELECT uuid, " + category + " FROM " + MySQL.getTableName(tableBaseName, duration) + " ORDER BY " + category + " DESC LIMIT " + playerCount);
        result.goToColumn(1);
        ...
        for (int i = 0; i < uuids.length; i++) {
            ...
            component = component
                    .append(Stats
                            .formatToplist(username, i + 1, value)
                            .clickEvent(ClickEvent.runCommand(cmd))
                            .hoverEvent(Component
                                    .text("Klicke um die Statistiken von ")
                                    .color(NamedTextColor.GRAY)
                                    .append(Component.text(username).color(NamedTextColor.GOLD))
                                    .append(Component.text(" anzuzeigen").color(NamedTextColor.GRAY))))
                    .appendNewline();
        }
        return component;
    }
    ...
    @Override public boolean execute(CommandSender sender, String[] args) {
        if (args.length == 2 || args.length == 3) {
            ...
            BukkitAdventureSupport
                    ...
                    .audienceProvider()
                    .sender(sender)
                    .sendMessage(getTopPlayers(MySQL.WOOLBATTLE_BASE_NAME, convertCategoryToMysql(category), 10, "/stats %name% " + duration, duration));
            return true;
        }
