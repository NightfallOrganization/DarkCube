...
public class CommandHeal extends EssentialsCommand {
	public CommandHeal() {
		super("heal", b -> b.executes(
						ctx -> heal(ctx.getSource(), Collections.singleton(ctx.getSource().asPlayer())))
				.then(Commands.argument("players", EntityArgument.players()).executes(
						ctx -> heal(ctx.getSource(), EntityArgument.getPlayers(ctx, "players")))));
	}
	...
	private static int heal(CommandSource source, Collection<Player> targets) {
		for (Player target : targets) {
			...
			UserAPI.getInstance().getUser(target).sendMessage(Message.YOU_WERE_HEALED);
		}
		if (!(source.getEntity() != null && 1 == targets.size()
				&& source.getEntity() instanceof Player && targets.contains(
