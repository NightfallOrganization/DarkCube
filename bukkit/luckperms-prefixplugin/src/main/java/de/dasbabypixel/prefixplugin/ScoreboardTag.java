...
class ScoreboardTag {
    ...
    public static ScoreboardTag getScoreboardTag(UUID uuid) {
        ...
        if (tag == null) {
            int weight = PrefixPluginBukkit.instance().getScoreboardManager().getUser(uuid).getCachedData().getMetaData().getPrefixes().keySet().stream().mapToInt(i -> i).max().orElse(Integer.MAX_VALUE);
    ...
    public String toString() {
        return Integer.MAX_VALUE - weight + String.valueOf(position);
    }
}
