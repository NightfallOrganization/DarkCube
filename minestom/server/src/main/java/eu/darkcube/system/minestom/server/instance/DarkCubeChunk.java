...
public class DarkCubeChunk extends Chunk {
    ...
    public DarkCubeChunk(@NotNull Instance instance, int chunkX, int chunkZ) {
    ...
    @Override public void setBiome(int x, int y, int z, @NotNull Biome biome) {
        ...
        section
                .biomePalette()
                .set(toSectionRelativeCoordinate(x) / 4, toSectionRelativeCoordinate(y) / 4, toSectionRelativeCoordinate(z) / 4, biome.id());
    }

    @Override public @NotNull List<Section> getSections() {
        return sections;
    }

    @Override public @NotNull Section getSection(int section) {
    ...
    @Override public @Nullable Block getBlock(int x, int y, int z, @NotNull Condition condition) {
        assertLock();
        if (y < minSection * CHUNK_SECTION_SIZE || y >= maxSection * CHUNK_SECTION_SIZE) return Block.AIR; // Out of bounds

        // Verify if the block object is present
        if (condition != Condition.TYPE) {
            final Block entry = !entries.isEmpty() ? entries.get(ChunkUtils.getBlockIndex(x, y, z)) : null;
        ...
        final int blockStateId = section
                .blockPalette()
    ...
    @Override public void sendChunk(@NotNull Player player) {
        if (!isLoaded()) return;
        player.sendPacket(chunkCache);
    }

    @Override public void sendChunk() {
        if (!isLoaded()) return;
        sendPacketToViewers(chunkCache);
    }

    @Override public @NotNull Chunk copy(@NotNull Instance instance, int chunkX, int chunkZ) {
        DarkCubeChunk dynamicChunk = new DarkCubeChunk(instance, chunkX, chunkZ);
    ...
    private synchronized @NotNull ChunkDataPacket createChunkPacket() {
        ...
        // Heightmap
        {
            ...
            heightmapsNBT = NBT.Compound(Map.of("MOTION_BLOCKING", NBT.LongArray(encodeBlocks(motionBlocking, bitsForHeight)), "WORLD_SURFACE", NBT.LongArray(encodeBlocks(worldSurface, bitsForHeight))));
        }
        // Data
        final byte[] data = ObjectPool.PACKET_POOL.use(buffer -> NetworkBuffer.makeArray(networkBuffer -> {
            for (Section section : sections) networkBuffer.write(section);
        }));
        return new ChunkDataPacket(chunkX, chunkZ, new ChunkData(heightmapsNBT, data, entries), createLightData());
    }
    ...
    @Override public @NotNull ChunkSnapshot updateSnapshot(@NotNull SnapshotUpdater updater) {
        ...
        return new SnapshotImpl.Chunk(minSection, chunkX, chunkZ, clonedSections, entries.clone(), entityIds, updater.reference(instance), tagHandler().readableCopy());
    }
