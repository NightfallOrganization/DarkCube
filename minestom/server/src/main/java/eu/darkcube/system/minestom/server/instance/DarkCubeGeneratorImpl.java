...
public class DarkCubeGeneratorImpl {
    ...
    static GenerationUnit section(Section section, int sectionX, int sectionY, int sectionZ, boolean fork) {
        ...
        final UnitModifier modifier = new DarkCubeGeneratorImpl.SectionModifierImpl(SECTION_SIZE, start, end, section.blockPalette(), section.biomePalette(), new Int2ObjectOpenHashMap<>(0), fork);
    ...
    static DarkCubeGeneratorImpl.UnitImpl chunk(Chunk chunk, int minSection, int maxSection, List<Section> chunkSections, int chunkX, int chunkZ) {
        final int minY = minSection * 16;
        AtomicInteger sectionCounterY = new AtomicInteger(minSection);
        List<GenerationUnit> sections = chunkSections
                .stream()
        ...
        final UnitModifier modifier = new DarkCubeGeneratorImpl.AreaModifierImpl(chunk, size, start, end, 1, sections.size(), 1, sections);
        return unit(modifier, start, end, sections);
    }

    static DarkCubeGeneratorImpl.UnitImpl chunk(int minSection, int maxSection, List<Section> chunkSections, int chunkX, int chunkZ) {
        return chunk(null, minSection, maxSection, chunkSections, chunkX, chunkZ);
    }

    static DarkCubeGeneratorImpl.UnitImpl chunk(Chunk chunk) {
        return chunk(chunk, chunk.getMinSection(), chunk.getMaxSection(), chunk.getSections(), chunk.getChunkX(), chunk.getChunkZ());
    }

    static DarkCubeGeneratorImpl.UnitImpl unit(UnitModifier modifier, Point start, Point end, List<GenerationUnit> divided) {
        ...
        return new DarkCubeGeneratorImpl.UnitImpl(modifier, size, start, end, divided, new CopyOnWriteArrayList<>());
    }
    ...
    sealed interface GenericModifier extends UnitModifier permits DarkCubeGeneratorImpl.AreaModifierImpl, DarkCubeGeneratorImpl.SectionModifierImpl {
        Point size();
        ...
        @Override default void setAll(@NotNull Supplier supplier) {
        ...
        @Override default void setAllRelative(@NotNull Supplier supplier) {
        ...
        @Override default void fill(@NotNull Block block) {
            fill(start(), end(), block);
        }

        @Override default void fill(@NotNull Point start, @NotNull Point end, @NotNull Block block) {
            final int endX = end.blockX();
            final int endY = end.blockY();
            final int endZ = end.blockZ();
            for (int x = start.blockX(); x < endX; x++) {
        ...
        @Override default void fillHeight(int minHeight, int maxHeight, @NotNull Block block) {
            final Point start = start();
            final Point end = end();
    ...
    static final class DynamicFork implements Block.Setter {
        ...
        @Override public void setBlock(int x, int y, int z, @NotNull Block block) {
            resize(x, y, z);
            GenerationUnit section = findAbsolute(sections, minSection, width, height, depth, x, y, z);
            assert section.absoluteStart().chunkX() == getChunkCoordinate(x) && section
                    .absoluteStart()
                    .section() == getChunkCoordinate(y) && section
                    .absoluteStart()
                    .chunkZ() == getChunkCoordinate(z) : "Invalid section " + section.absoluteStart() + " for " + x + ", " + y + ", " + z;
        ...
        private void resize(int x, int y, int z) {
            ...
            if (sections == null) {
                ...
                // Resize necessary
                final Vec newMin = new Vec(Math.min(minSection.x(), sectionX * 16), Math.min(minSection.y(), sectionY * 16), Math.min(minSection.z(), sectionZ * 16));
                final Vec newMax = new Vec(Math.max(minSection.x() + width * 16, sectionX * 16 + 16), Math.max(minSection.y() + height * 16, sectionY * 16 + 16), Math.max(minSection.z() + depth * 16, sectionZ * 16 + 16));
    ...
    record UnitImpl(UnitModifier modifier, Point size, Point absoluteStart, Point absoluteEnd, List<GenerationUnit> divided,
                    List<DarkCubeGeneratorImpl.UnitImpl> forks) implements GenerationUnit {
        @Override public @NotNull GenerationUnit fork(@NotNull Point start, @NotNull Point end) {
        ...
        @Override public void fork(@NotNull Consumer<Block.@NotNull Setter> consumer) {
            DarkCubeGeneratorImpl.DynamicFork dynamicFork = new DarkCubeGeneratorImpl.DynamicFork();
        ...
        @Override public @NotNull List<GenerationUnit> subdivide() {
        ...
        private GenerationUnit registerFork(Point start, List<GenerationUnit> sections, int width, int height, int depth) {
            ...
            final Point size = end.sub(start);
            final DarkCubeGeneratorImpl.AreaModifierImpl modifier = new DarkCubeGeneratorImpl.AreaModifierImpl(null, size, start, end, width, height, depth, sections);
            final DarkCubeGeneratorImpl.UnitImpl fork = new DarkCubeGeneratorImpl.UnitImpl(modifier, size, start, end, sections, forks);
            forks.add(fork);
            return fork;
        }
    }

    record SectionModifierImpl(Point size, Point start, Point end, Palette blockPalette, Palette biomePalette, Int2ObjectMap<Block> cache,
                               boolean fork) implements DarkCubeGeneratorImpl.GenericModifier {
        @Override public void setBiome(int x, int y, int z, @NotNull Biome biome) {
            ...
            this.biomePalette.set(toSectionRelativeCoordinate(x) / 4, toSectionRelativeCoordinate(y) / 4, toSectionRelativeCoordinate(z) / 4, biome.id());
        }
        ...
        @Override public void setRelative(int x, int y, int z, @NotNull Block block) {
            handleCache(x, y, z, block);
            this.blockPalette.set(x, y, z, retrieveBlockId(block));
        }

        @Override public void setAllRelative(@NotNull Supplier supplier) {
        ...
        @Override public void fill(@NotNull Block block) {
        ...
        @Override public void fillBiome(@NotNull Biome biome) {
            if (fork) throw new IllegalStateException("Cannot modify biomes of a fork");
    ...
    record AreaModifierImpl(Chunk chunk, Point size, Point start, Point end, int width, int height, int depth,
                            List<GenerationUnit> sections) implements DarkCubeGeneratorImpl.GenericModifier {
        @Override public void setBlock(int x, int y, int z, @NotNull Block block) {
            checkBorder(x, y, z);
        ...
        @Override public void setAll(@NotNull Supplier supplier) {
        ...
        @Override public void fillHeight(int minHeight, int maxHeight, @NotNull Block block) {
            final Point start = this.start;
