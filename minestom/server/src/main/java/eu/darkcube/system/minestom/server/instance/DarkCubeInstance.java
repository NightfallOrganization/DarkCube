...
public class DarkCubeInstance extends Instance {
    ...
    public final Map<Long, CompletableFuture<Chunk>> loadingChunks = new ConcurrentHashMap<>();
    ...
    protected DarkCubeInstance srcInstance; // only present if this instance has been created using a copy
    Map<Long, List<DarkCubeGeneratorImpl.SectionModifierImpl>> generationForks = new ConcurrentHashMap<>();
    ...
    public DarkCubeInstance(@NotNull UUID uniqueId, @NotNull DimensionType dimensionType, @Nullable IChunkLoader loader) {
    ...
    @Override public boolean placeBlock(@NotNull BlockHandler.Placement placement) {
        final Point blockPosition = placement.getBlockPosition();
        final Chunk chunk = getChunkAt(blockPosition);
        if (!isLoaded(chunk)) return false;
        UNSAFE_setBlock(chunk, blockPosition.blockX(), blockPosition.blockY(), blockPosition.blockZ(), placement.getBlock(), placement, null);
        return true;
    }

    @Override public boolean breakBlock(@NotNull Player player, @NotNull Point blockPosition, @NotNull BlockFace blockFace) {
        final Chunk chunk = getChunkAt(blockPosition);
        ...
        if (allowed) {
            ...
            UNSAFE_setBlock(chunk, x, y, z, resultBlock, null, new BlockHandler.PlayerDestroy(block, this, blockPosition, player));
            // Send the block break effect packet
            PacketUtils.sendGroupedPacket(chunk.getViewers(), new EffectPacket(2001 /*Block break + block break sound*/, blockPosition, block.stateId(), false),
    ...
    @Override public @NotNull CompletableFuture<Chunk> loadChunk(int chunkX, int chunkZ) {
        return loadOrRetrieve(chunkX, chunkZ, () -> retrieveChunk(chunkX, chunkZ));
    }

    @Override public @NotNull CompletableFuture<Chunk> loadOptionalChunk(int chunkX, int chunkZ) {
    ...
    @Override public void unloadChunk(int chunkX, int chunkZ) {
        super.unloadChunk(chunkX, chunkZ);
    }
    ...
    protected @NotNull CompletableFuture<@NotNull Chunk> retrieveChunk(int chunkX, int chunkZ) {
        ...
        final Runnable retriever = () -> loader.loadChunk(this, chunkX, chunkZ).thenCompose(chunk -> {
                    if (chunk != null) {
    ...
    protected @NotNull CompletableFuture<@NotNull Chunk> createChunk(int chunkX, int chunkZ) {
        ...
        if (generator != null && chunk.shouldGenerate()) {
            ...
            ForkJoinPool.commonPool().submit(() -> {
                var chunkUnit = DarkCubeGeneratorImpl.chunk(chunk);
                try {
                    // Generate block/biome palette
                    generator.generate(chunkUnit);
                    // Apply nbt/handler
                    if (chunkUnit.modifier() instanceof DarkCubeGeneratorImpl.AreaModifierImpl chunkModifier) {
                        for (var section : chunkModifier.sections()) {
                            if (section.modifier() instanceof DarkCubeGeneratorImpl.SectionModifierImpl sectionModifier) {
                    ...
                    for (var fork : chunkUnit.forks()) {
                        var sections = ((DarkCubeGeneratorImpl.AreaModifierImpl) fork.modifier()).sections();
                        for (var section : sections) {
                            if (section.modifier() instanceof DarkCubeGeneratorImpl.SectionModifierImpl sectionModifier) {
                                if (sectionModifier.blockPalette().count() == 0) continue;
                                ...
                                if (forkChunk != null) {
                                    ...
                                    // Update players
                                    if (forkChunk instanceof DarkCubeChunk dynamicChunk) {
    ...
    private void applyFork(@NotNull Chunk chunk, DarkCubeGeneratorImpl.SectionModifierImpl sectionModifier) {
        synchronized (chunk) {
    ...
    private void applyGenerationData(Chunk chunk, DarkCubeGeneratorImpl.SectionModifierImpl section) {
     ...
     */
    public synchronized DarkCubeInstance copy() {
        DarkCubeInstance copiedInstance = new DarkCubeInstance(UUID.randomUUID(), getDimensionType());
     ...
     */
    public @Nullable DarkCubeInstance getSrcInstance() {
        return srcInstance;
    }
        ...
        for (int offsetX = -1; offsetX < 2; offsetX++) {
            for (int offsetY = -1; offsetY < 2; offsetY++) {
                for (int offsetZ = -1; offsetZ < 2; offsetZ++) {
                    ...
                    if (neighborY < getDimensionType().getMinY() || neighborY > getDimensionType().getTotalHeight()) continue;
                    final Block neighborBlock = cache.getBlock(neighborX, neighborY, neighborZ, Condition.TYPE);
                    if (neighborBlock == null) continue;
                    final BlockPlacementRule neighborBlockPlacementRule = MinecraftServer
                            .getBlockManager()
                            .getBlockPlacementRule(neighborBlock);
                    if (neighborBlockPlacementRule == null) continue;

                    final Vec neighborPosition = new Vec(neighborX, neighborY, neighborZ);
                    final Block newNeighborBlock = neighborBlockPlacementRule.blockUpdate(this, neighborPosition, neighborBlock);
