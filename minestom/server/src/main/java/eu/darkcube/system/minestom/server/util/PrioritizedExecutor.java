...
public interface PrioritizedExecutor {
    ...
    interface RejectedExecutionHandler {
         ...
         */
        void rejectedExecution(Runnable r, Pool executor);
    }

    class Default implements PrioritizedExecutor {

        private final Pool executor;
        private final ChunkPriorityQueue<Runnable> queue;

        public Default(int priorityCount) {
            int maxSize = Runtime.getRuntime().availableProcessors();
            this.queue = new ChunkPriorityQueue<>(priorityCount);
     ...
     * invokes the {@link
     * RejectedExecutionHandler#rejectedExecution(Runnable, Pool)}
     ...
     * @since 1.5
     */
    class Pool extends AbstractExecutorService {
         ...
         * expire.
         */
        private final ChunkPriorityQueue<Runnable> workQueue;
        /**
         ...
         */
        public Pool(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit, ChunkPriorityQueue<Runnable> workQueue, ThreadFactory threadFactory, RejectedExecutionHandler handler) {
         ...
         */
        public CancellableTask executeTask(Runnable command) {
            if (command == null) throw new NullPointerException();
            if (!(command instanceof Prioritized p)) throw new IllegalArgumentException();
            /*
             * Proceed in 3 steps:
             *
            ...
            if (workerCountOf(c) < corePoolSize) {
                if (addWorker(command, true)) return CancellableTask.NOOP;
                c = ctl.get();
            }
            if (isRunning(c)) {
                var entry = workQueue.add(p.priority(), command);
                int recheck = ctl.get();
        ...
        public static class CallerRunsPolicy implements RejectedExecutionHandler {
             ...
             */
            public void rejectedExecution(Runnable r, Pool e) {
                if (!e.isShutdown()) {
                    r.run();
                }
            }
         ...
         * <p>
         * This is the default handler for {@link Pool} and
