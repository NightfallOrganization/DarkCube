...
/**
 * From Papers SingleUserAreaMap
 * @param <T>
 */
abstract class AreaMap<T> {
    ...
    public AreaMap(final T parameter) {
    ...
    public final boolean update(final int toX, final int toZ, final int newViewDistance) {
        ...
        final int totalX = branchlessAbs(fromX - toX);
        final int totalZ = branchlessAbs(fromZ - toZ);
        ...
        if (oldViewDistance != newViewDistance) {
            ...
            for (int currX = oldMinX; currX <= oldMaxX; ++currX) {
                for (int currZ = oldMinZ; currZ <= oldMaxZ; ++currZ) {
                    ...
                    if (Math.max(branchlessAbs(currX - toX), branchlessAbs(currZ - toZ)) > newViewDistance) {
            ...
            for (int currX = newMinX; currX <= newMaxX; ++currX) {
                for (int currZ = newMinZ; currZ <= newMaxZ; ++currZ) {
                    ...
                    if (Math.max(branchlessAbs(currX - fromX), branchlessAbs(currZ - fromZ)) > oldViewDistance) {
    ...
    private static int branchlessAbs(final int val) {
        // -n = -1 ^ n + 1
