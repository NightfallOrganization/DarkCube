...
public class DoubleJumpPerk extends Perk {
	...
	public static class ListenerDoubleJump extends PerkListener {
		...
		public void handle(PlayerToggleFlightEvent e) {
			...
			if (p.getGameMode() != GameMode.SURVIVAL || user.isTrollMode() || !user.getTeam()
					.canPlay()) {
				return;
			}
			...
			if (res.state == RefreshResult.ALLOWED && e.isFlying()) {
				...
				Vector velo = p.getLocation().getDirection().setY(0).normalize().multiply(0.11)
						.setY(1.05);
		...
		public void handle(PlayerGameModeChangeEvent e) {
			new Scheduler(() -> workRefresh(WBUser.getUser(e.getPlayer()))).runTask();
		}

		@EventHandler
		public void handle(SpiderStateChangeEvent event) {
			new Scheduler(() -> workRefresh(event.user())).runTask();
		}
		...
		private static class RefreshResult {
			private static final int INSUFFICIENT_WOOL = 0, EVENT_CANCELLED = 1, ALLOWED = 2,
					DENIED = 3;
	...
	public static class DoubleJumpUserPerk extends DefaultUserPerk {
		...
		public void cooldown(int cooldown) {
			...
			if (cooldown <= 0 && cooldown() > 0) {
				if (!owner().getBukkitEntity().getAllowFlight())
