...
public class SpiderPerk extends Perk {
	...
	public SpiderPerk() {
		super(ActivationType.PASSIVE, SPIDER, new Cooldown(Unit.TICKS, 60), 2, Item.PERK_SPIDER,
				(user, perk, id, perkSlot) -> new CooldownUserPerk(user, id, perkSlot, perk,
						Item.PERK_SPIDER_COOLDOWN));
		SpiderSchedulerListener schedulerListener = new SpiderSchedulerListener();
		addScheduler(schedulerListener);
		addListener(schedulerListener);
	}

	public static boolean isClimbing(WBUser user) {
	...
	private class SpiderSchedulerListener extends Scheduler
			implements ConfiguredScheduler, Listener {

		@Override
		public void run() {
			float woolPerTick = cost() / 20F;
			WBUser.onlineUsers().stream().filter(u -> u.getTeam().canPlay()).forEach(user -> {
				user.perks().perks(perkName()).forEach(perk -> {
					...
					if ((dir.getX() == 0 && dir.getZ() == 0) || user.getBukkitEntity().isOnGround()
							|| user.getBukkitEntity().isSneaking() || wool < woolPerTick) {
						if (isClimbing(user)) {
							interrupt(user);
						}
						return;
					...
					dir.normalize();
					loc.add(dir);
					if (loc.getBlock().getType().isSolid() || loc.clone().add(0, 1, 0).getBlock()
							.getType().isSolid()) {
						if (!isClimbing(user)) {
		...
		private boolean interrupt(WBUser user) {
			if (isClimbing(user)) {
