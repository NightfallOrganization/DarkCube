...
public class CommandSetNameColor extends WBCommandExecutor {
	public CommandSetNameColor() {
		super("setNameColor", b -> b.then(Commands.argument("nameColor", EnumArgument.enumArgument(
				...
				.executes(ctx -> {
					...
					ChatColor nameColor =
							EnumArgument.getEnumArgument(ctx, "nameColor", ChatColor.class);
					team.setNameColor(nameColor);
					ctx.getSource().sendMessage(LegacyComponentSerializer.legacySection()
							.deserialize("§7Du hast die Namensfarbe des Teams "
									+ team.getDisplayNameKey() + " zu " + nameColor
									+ nameColor.name() + "§7 geändert."));
					return 0;
				})));
	}
	//
