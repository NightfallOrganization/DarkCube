...
public class CommandSetNameColor extends WBCommand {
    public CommandSetNameColor() {
        // @formatter:off
        super("setNameColor", b -> b
                .then(Commands.argument("nameColor", EnumArgument.enumArgument(Arrays.stream(ChatColor.values()).filter(ChatColor::isColor).toArray(ChatColor[]::new), c -> new String[]{c.name().toLowerCase()}))
                        .executes(ctx -> {
                            // @formatter:on
