...
public class DefaultPlayerPerks implements PlayerPerks {
    ...
    public DefaultPlayerPerks(Map<ActivationType, PerkName[]> perks, Map<ActivationType, int[]> perkSlots) {
        this();
        perks(perks);
        for (ActivationType type : perkSlots.keySet()) {
            System.arraycopy(perkSlots.get(type), 0, this.perkSlots.get(type), 0, Math.min(perkSlots.get(type).length, this.perkSlots.get(type).length));
        }
    }
        ...
        for (ActivationType type : list) {
            for (int i = 0; i < perkSlots.get(type).length; i++) {
                ...
                if (type == ActivationType.PRIMARY_WEAPON && i == 0) {
                    ...
                    PerkName name = null;
                    for (Map.Entry<PerkName, Perk> e : WoolBattleBukkit.instance().perkRegistry().perks().entrySet()) {
