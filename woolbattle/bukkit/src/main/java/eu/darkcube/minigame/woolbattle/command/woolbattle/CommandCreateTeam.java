...
public class CommandCreateTeam extends WBCommandExecutor {
    public CommandCreateTeam(WoolBattle woolbattle) {
        super("createTeam", b -> b
                .then(argument("team", StringArgument.string(Arrays.asList(SupportedColors.values()).stream().map(SupportedColors::name).toArray(String[]::new)))
                        .then(argument("weight", IntegerArgumentType.integer())
                                .then(argument("woolColor", EnumArgument.enumArgument(DyeColor.values(), d -> new String[]{d.name().toLowerCase()}))
                                        .then(argument("nameColor", EnumArgument.enumArgument(SupportedColors.values()))
