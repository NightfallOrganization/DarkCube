...
public class CommandFriend extends Command implements TabExecutor {
    ...
    public CommandFriend() {
        super("friend", "bungeecord.command.friend", "freund");
    }

    public PlayerManager getManager() {
        return getInstance().getServicesRegistry().getFirstService(IPlayerManager.class);
    }
            ...
            if (cmd.equalsIgnoreCase("list")) {
                ...
                if (friendsOff.size() == 0) {
                    ...
                    for (ICloudOfflinePlayer off : friendsOff) {
                        ...
                        Main.sendMessage(sender, "§7" + off.getName() + " §7(" + (on != null ? "§aOnline" : "§cOffline") + "§7)");
                    }
                    Main.sendMessage(sender, "§8---------------------------");
                }
                return;
                ...
                if (args.length == 2) {
                    ...
                    if (FriendUtil.getPending(p.getUniqueId()).contains(offline.getUniqueId())) {
                        execute(sender, new String[]{"accept", offline.getName()});
                        return;
                    }
                    Set<UUID> friends = FriendUtil.getFriends(p.getUniqueId());
                    ...
                    if (friends.contains(offline.getUniqueId())) {
                        ...
                        Main.sendMessage(sender, "§cDu hast diesem Spieler bereits eine Freundschaftsanfrage geschickt!");
                        return;
                    }
                    ...
                    if (online != null) {
                        ...
                        Main.sendMessage(Main.getInstance().getProxy().getPlayer(offline.getUniqueId()), new ComponentBuilder()
                                .append("§aDer Spieler §6" + p.getName() + " §ahat dir eine Freundschaftsanfrage geschickt!\n")
                                .append(accept)
                                .append(" ")
                                .append(deny)
                                .create());
                    ...
                    Main.sendMessage(sender, "§aDu hast dem Spieler §6" + offline.getName() + " §aeine Freundschaftsanfrage geschickt!");
                    return;
                }
                ...
                if (args.length == 2) {
                    ...
                    if (online != null) {
                        Main.sendMessage(Main
                                .getInstance()
                                .getProxy()
                                .getPlayer(online.getUniqueId()), "§6" + p.getName() + " §chat dich als Freund entfernt!");
                    }
                    Main.sendMessage(sender, "§cDu hast den Spieler §6" + offline.getName() + " §caus deiner Freundesliste entfernt!");
                    return;
                }
                ...
                if (args.length == 2) {
                    ...
                    if (online != null) {
                        Main.sendMessage(Main
                                .getInstance()
                                .getProxy()
                                .getPlayer(online.getUniqueId()), "§6" + p.getName() + " §chat seine Freundschaftsanfrage zurückgezogen!");
                    }
                    Main.sendMessage(sender, "§cDu hast die Freundschaftsanfrage an §6" + offline.getName() + " §czurückgezogen!");
                    return;
                }
                ...
                if (args.length == 2) {
                    ...
                    if (online != null) {
                        Main.sendMessage(Main
                                .getInstance()
                                .getProxy()
                                .getPlayer(online.getUniqueId()), "§6" + p.getName() + " §ahat deine Freundschaftsanfrage angenommen!");
                    }
                    Main.sendMessage(sender, "§aDu hast die Freundschaftsanfrage von §6" + offline.getName() + " §aangenommen!");
                    return;
                }
                ...
                if (args.length == 2) {
                    ...
                    if (online != null) {
                        Main.sendMessage(Main
                                .getInstance()
                                .getProxy()
                                .getPlayer(online.getUniqueId()), "§6" + p.getName() + " §chat deine Freundschaftsanfrage abgelehnt!");
                    }
                    Main.sendMessage(sender, "§cDu hast die Freundschaftsanfrage von §6" + offline.getName() + " §cabgelehnt!");
                    return;
                }
                ...
                if (pendingOff.size() == 0) {
                    ...
                    for (ICloudOfflinePlayer off : pendingOff) {
                        ICloudPlayer on = getManager().getOnlinePlayer(off.getUniqueId());
                        TextComponent base = new TextComponent("§7" + off.getName() + " §7(" + (on != null ? "§aOnline" : "§cOffline") + "§7)");
                        TextComponent accept = new TextComponent("§8[§aAnnehmen§8]");
                ...
                FriendUtil.setAcceptsFriendRequests(p.getUniqueId(), !FriendUtil.acceptsFriendRequests(p.getUniqueId()));
    ...
    @Override public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            return Arrays.toSortedStringList(new String[]{"list", "add", "remove", "cancel", "accept", "deny", "requests", "jump", "toggle"}, args[0]);
